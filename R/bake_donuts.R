...
#' @param donut_incoming Should the donuts display incoming data instead of outgoing?
#' @param edge_incoming_only for one way show one line to the source
#' @param home4all should the donut plot show the home data in the same colour for all, regardless of highlights?
...
bake_donuts = function(x,
  ...
  basemaps = c("Esri.WorldGrayCanvas", "OpenStreetMap")
	...
	# defines how long the line is for incoming
	
	# is incoming next to source or arrival  
	if (edge_incoming) {
		edge_range = c(.5, 1)
	...
	# if only want incoming (e.g. for observation sites)
	if(edge_incoming_only){
		edge_range = c(0, 1)
		edge_trunc = units::set_units(c(500, 0), "m")
	}
	...
	if(!donut_incoming){
		uAbs = x$E %>% 
			...
			select(name = name_from, class_to, flow) %>% 
			ungroup() %>% 
			complete(name, class_to, fill = list(flow = 0)) %>% 
			pivot_wider(names_from = class_to, values_from = flow) 
		uAbs$total_out_ = rowSums(uAbs[, c(highlight, "other_")])
	} else {
		uAbs = x$E %>% 
			group_by(name_to, class_from) %>% 
			summarize(flow = sum(flow)) %>%  #round_num(sum(flow), round_to)) %>% 
			select(name = name_to, class_from, flow) %>%
			ungroup() %>% 
			complete(name, class_from, fill = list(flow = 0)) %>% 
			pivot_wider(names_from = class_from, values_from = flow) 
		uAbs$total_in_ = rowSums(uAbs[, c(highlight, "other_")])
	}
	...
	if(!donut_incoming){
		...
		uRes[,-1] = (uRes[,-1] / (uRes$stay_ + uRes$total_in_)) * 100
	}
	...
	if(!donut_incoming){
		...
		uFrom[,-1] = mapply(function(d1, d2, is_item) {
			p = ifelse(d2 > 0 & d2 < 0.5, "(<1%)", ifelse(d2 > 99.5 & d2 < 100, "(>99%)", paste0("(", round(d2), "%)")))
	...
	if(!home4all){
		...
		for (h in highlight) {
			id = which(uRes$name == h)
			stopifnot(length(id) == 1)
			uRes[[h]][id] = uRes$stay_[id]
			uRes$stay_[id] = 0
		}
	}
	grobU = x$U %>% left_join(uRes, by = c("name" = "name"))
	...
	# Maps each city to a category (highlight, "other_", or "stay_").
	
	
	# This function:
	#   
	#   Converts OD data into a long format.
	# 
	# Creates donut charts (polar bar charts) using ggplot2.
	# 
	# Generates graphical objects (grobs) for each location.
	...
	# Filters out small flows to avoid clutter.
	# 
	# Creates two versions:
	#   
	#   x_half: Keeps only significant flows.
	# 
	# x_full: Keeps more flows, but with a lower threshold.
	...
	if(!donut_incoming){
		...
		lns <- x$E %>%
			#filter(show, name != name) 
			mutate(width = flow_buffer + pmin(flow, flow_max)) %>%
			select(label, flow, width, class_from = class_to, class_to = class_from) %>%
			arrange(desc(flow))
	}
	...
	if(!donut_incoming){
		x$U <- x$U %>%
			left_join(uFor, by = c("name" = "name"))
	} else{
		x$U <- x$U %>%
			left_join(uFrom, by = c("name" = "name"))
	}
	...
	if(!donut_incoming){
		...
		vars = c("residents_", "stay_", "total_in_", highlight, "other_", "flow_in")
		names(vars) = c(text$popup_residents, text$popup_stay, text$popup_outflow, paste0(text$popup_to, highlight), text$popup_other, text$popup_inflow)
	}
	...
	# Base map (tm_basemap): Provides background context.
	# Lines (tm_lines): Draws flows with thickness based on flow values.
	# Symbols (tm_symbols): Represents units as donut charts.
	# Text (tm_text): Labels locations with names.
